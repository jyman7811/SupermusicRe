...
class LeaveGuildHandler : ListenerAdapter() {
    override fun onGuildVoiceUpdate(event: GuildVoiceUpdateEvent) {
        ...
        if (channel.members.size == 0) return ControllerManager.musicManager.destroy(event.guild.idLong, "모든 사용자가 나가서 연결을 해제 하였습니다!")

        /**
         * 채널에 봇 혼자 남았을 때 컨트롤러를 삭제합니다.
         */
        if (channel.members.size == 1 && channel.members[0].user == event.jda.selfUser) {
            ControllerManager.musicManager.destroy(event.guild.idLong, "모든 사용자가 나가서 연결을 해제 하겠습니다!")
        }
    }
}